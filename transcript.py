...
class Transcript:
    ...
    def empty_interval(self):
        if self.on_positive_strand():
            ...
            return PieceWiseLocation.singleton(self.chromosome, self.on_positive_strand(),
                                               self.tx_end - 1, self.tx_end - 1)
    ...
    def __init__(self, gene_id, chromosome, strand, tx_start, tx_end, cds_start, cds_end, exon_count, exon_starts,
                 exon_ends, gene_name):
    ...
    def __str__(self):
        return "{0:s}\t{1:d}\t{2:d}\t{3:s}\t{4:s}".format(self.chromosome, self.tx_start, self.tx_end, self.gene_id,
                                                          self.strand)
    ...
    def tss(self):
        if self.on_positive_strand():
            return PieceWiseLocation.singleton(self.chromosome, self.on_positive_strand(), self.tx_start,
                                               self.tx_start + 1)
        else:
    ...
    def tss_shifted_1bp_upstream(self):
        if self.on_positive_strand():
            return PieceWiseLocation.singleton(self.chromosome, self.on_positive_strand(), self.tx_start - 1,
                                               self.tx_start)
        else:
