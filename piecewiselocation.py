...
class PieceWiseLocation:    
    ...
    def __str__(self):
        if self.on_positive_strand:
            ...
            for interval in self.intervals[1:]: string += "," + str(interval)
            string += " 3' (+, 0-based)"
            return string
        else:
            ...
            for idx, element in enumerate(result_intervals[:]):
                if element.overlaps_with(subtract_interval):
                    ...
                    if subtract_interval.start in element:
                        ...
                        if not s.isempty(): result_intervals.append(s)
    ...
    def __iter__(self):
        for interval in self.intervals: yield interval
    
    
    def __contains__(self, element):
        for interval in self:
            if element in interval: return True
        return False
    ...
    def extend_upstream(self, number_of_bases, chromosome2length):
        ...
        if self.on_positive_strand:
            ...
            intervals.append(Interval(first_upstream_element.start, min(first_upstream_element.end + number_of_bases, chromosome_length)))
            return PieceWiseLocation(self.chromosome, self.on_positive_strand, intervals)
    ...
    def extend_downstream(self, number_of_bases, chromosome2length):
        ...
        if self.on_positive_strand:
            ...
            intervals = self.intervals[0:-1]
            chromosome_length = chromosome2length[self.chromosome]
            intervals.append(Interval(first_downstream_element.start, min(first_downstream_element.end + number_of_bases, chromosome_length)))
            return PieceWiseLocation(self.chromosome, self.on_positive_strand, intervals)
        else:
            first_downstream_element = self.intervals[0]
            intervals = [Interval(max(0, first_downstream_element.start - number_of_bases), first_downstream_element.end)]
            intervals.extend(self.intervals[1:])
            return PieceWiseLocation(self.chromosome, self.on_positive_strand, intervals)
    ...
    def interval_limit(self, limit):
        ...
        for interval in self.intervals:
            ...
            if not limited_interval.isempty(): intervals.append(limited_interval)
        return PieceWiseLocation(self.chromosome, self.on_positive_strand, intervals)